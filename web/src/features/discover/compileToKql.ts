...
//
// Note: /v1/query is scoped to ONE database (X-Database header). Callers are
// responsible for ensuring every source in `sources` shares the same db
// prefix; sources from other databases must be dropped before calling here.
...
export function compileToKql(
  ...
  // Normalise: accept a single source object for backwards compat.
  const arr = Array.isArray(sources) ? sources : [sources];

  if (arr.length === 0) return "";

  // 0-length key means "nothing selected" — single-source early exit.
  if (arr.length === 1) {
    const s = arr[0];
    if (!s.key) return "";
  ...
  // ── 2+ sources: union withsource=_source (branch1), (branch2) | take 500 ─
  // Each branch uses its OWN timestampColumn and stringColumns.
  // The trailing | take 500 is emitted once, after the union.
...
/** Build one branch pipeline (for the union case): tableExpr + where clauses.
 * Does NOT include | take — take is appended once after the full union. */
function oneBranch(
  source: SourceKey,
  timestampColumn: string | null,
  stringColumns: string[],
...
/** Build a single-source pipe including the trailing | take 500. */
function onePipe(
  source: SourceKey,
  timestampColumn: string | null,
  stringColumns: string[],

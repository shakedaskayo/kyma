...
/** Copy-paste clone block. The default copy uses a `<PENSIEVE_TOKEN>` placeholder;
...
export function CloneInstructions({ name }: { name: string }) {
  ...
  const copy = (withToken: boolean) => {
    const text = withToken
      ? `git clone ${url.replace("://", `://pensieve:${token}@`)}`
      : `git clone ${url}`;
    ...
    <div className="rounded-lg border bg-muted/30 p-3">
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs font-medium text-muted-foreground">
          Clone this brain — password is a pensieve API token
        </p>
      ...
      <pre className="mt-2 overflow-x-auto rounded bg-background/80 px-3 py-2 font-mono text-xs">
        {`git clone ${url}\n# username: pensieve · password: <PENSIEVE_TOKEN>`}
      </pre>
      <p className="mt-2 text-2xs text-muted-foreground">
        ...
        <code>pensieve brain clone {name}</code>.
      </p>
    </div>
  );
}

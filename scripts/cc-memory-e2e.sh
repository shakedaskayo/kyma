#!/usr/bin/env bash
# End-to-end test for the pensieve ⇄ Claude Code file-memory sync.
#
# Exercises the FULL lifecycle against the real binary in a throwaway
# sandbox (synthetic ~/.claude home + isolated pensieve store — your real
# files are never touched):
#
#   ingest (frontmatter→nodes, wikilinks→edges, realm via transcript cwd)
#   → recall over file memories (MCP memory_search)
#   → idempotent re-scan
#   → promotion (save_memory → native pensieve-*.md + managed MEMORY.md region)
...
#
# Prereqs: jq; `cargo build -p pensieve-cli` already run (or PENSIEVE=<path>).
...
cd "$REPO"
PENSIEVE="${PENSIEVE:-$REPO/target/debug/pensieve}"
command -v jq >/dev/null || { echo "jq required"; exit 1; }
[ -x "$PENSIEVE" ] || { echo "pensieve binary not found at $PENSIEVE (cargo build -p pensieve-cli)"; exit 1; }

# ------------------------------------------------------------------
# Sandbox
# ------------------------------------------------------------------
SANDBOX="$(mktemp -d /tmp/pensieve-cc-e2e.XXXXXX)"
trap 'rm -rf "$SANDBOX"' EXIT
export PENSIEVE_HOME="$SANDBOX/pensieve"
export PENSIEVE_CC_HOME="$SANDBOX/cc"
# Reuse an already-downloaded model cache when present (faster, offline-safe).
mkdir -p "$PENSIEVE_HOME"
[ -d "$REPO/.fastembed_cache" ] && cp -R "$REPO/.fastembed_cache" "$PENSIEVE_HOME/.fastembed_cache"

PROJ="$SANDBOX/work/e2e-proj"
SLUG="$(printf '%s' "$PROJ" | sed -E 's/[^A-Za-z0-9]/-/g')"
MEM="$PENSIEVE_CC_HOME/projects/$SLUG/memory"
mkdir -p "$MEM" "$PENSIEVE_CC_HOME/sessions" "$PROJ"
# Realm resolution falls back to the transcript cwd for unknown slugs.
echo "{\"cwd\":\"$PROJ\"}" > "$PENSIEVE_CC_HOME/projects/$SLUG/0000-session.jsonl"
...
mcp() {
  printf '%s\n{"jsonrpc":"2.0","id":2,"method":"tools/call","params":%s}\n' "$INIT" "$1" \
    | PENSIEVE_CC_SYNC_ON_MCP=0 "$PENSIEVE" mcp 2>/dev/null | tail -1
}
sql() { mcp "{\"name\":\"run_sql\",\"arguments\":{\"database\":\"memory\",\"sql\":$(jq -Rs . <<<"$1")}}" \
        | jq -r '.result.structuredContent'; }
sync_out() { "$PENSIEVE" sync --cc-only 2>&1 | grep 'cc-sync' || true; }
...
step "4. promotion: high-value pensieve memories become native files"
...
out="$(sync_out)"; echo "  $out"
check "two promoted files written"       test -f "$MEM/pensieve-staging-gate-rule.md" -a -f "$MEM/pensieve-error-message-convention.md"
check "promoted file is pensieve-stamped"    grep -q "source: pensieve" "$MEM/pensieve-staging-gate-rule.md"
check "user MEMORY.md line intact"       grep -q "\[Auth model\](auth-model.md)" "$MEM/MEMORY.md"
check "managed region lists promotions"  grep -q "pensieve-staging-gate-rule.md" "$MEM/MEMORY.md"
check "managed markers present"          grep -q "BEGIN pensieve-managed" "$MEM/MEMORY.md"

step "5. user edits a promoted file → pensieve pulls it back, stops rewriting"
sed -i '' 's/never tag before/NEVER EVER tag before/' "$MEM/pensieve-staging-gate-rule.md" 2>/dev/null \
  || sed -i 's/never tag before/NEVER EVER tag before/' "$MEM/pensieve-staging-gate-rule.md"
out="$(sync_out)"; echo "  $out"
...
out="$(sync_out)"
check "file not rewritten on the next pass" grep -q "NEVER EVER" "$MEM/pensieve-staging-gate-rule.md"
...
NOW_MS=$(( $(date +%s) * 1000 ))
echo "{\"pid\":1,\"cwd\":\"$PROJ\",\"updatedAt\":$NOW_MS,\"status\":\"busy\"}" > "$PENSIEVE_CC_HOME/sessions/live.json"
...
out="$(sync_out)"; echo "  $out"
check "no file written while session active" test ! -f "$MEM/pensieve-quiet-probe.md"
check "deferral recorded in audit log" grep -q '"skipped_quiet":true' "$PENSIEVE_HOME/cc-curation.log"
rm "$PENSIEVE_CC_HOME/sessions/live.json"
out="$(sync_out)"
check "written after the session ends" test -f "$MEM/pensieve-quiet-probe.md"

step "9. plugin session hooks fire the sync (detached, fail-open)"
export PATH="$(dirname "$PENSIEVE"):$PATH"
echo '{"session_id":"e2e","source":"startup"}' \
  | bash integrations/claude-code/pensieve-memory/scripts/on-session-start.sh >/dev/null 2>&1
...
echo '{"session_id":"e2e","reason":"exit"}' \
  | bash integrations/claude-code/pensieve-memory/scripts/on-session-end.sh >/dev/null 2>&1
...
check "audit log has the full history" test "$(wc -l < "$PENSIEVE_HOME/cc-curation.log")" -ge 5

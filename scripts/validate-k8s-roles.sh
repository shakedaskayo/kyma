...
# (KYMA_INGEST_MODE=staged) so the committer actually has work to drain. It
# proves:
...
# (KYMA_IMAGE, default kyma-smoke-kyma:latest).

set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; cd "$ROOT"

CLUSTER="kyma-roles-smoke"
NS="kyma-roles"
SRC_IMAGE="${KYMA_IMAGE:-kyma-smoke-kyma:latest}"
IMAGE_REPO="kyma-engine"; IMAGE_TAG="roles"
LPORT="${KYMA_ROLES_PORT:-18092}"
...
cleanup() {
  ...
  if [[ "${KYMA_KEEP_CLUSTER:-0}" == "1" ]]; then
    echo "  KYMA_KEEP_CLUSTER=1 — leaving cluster '$CLUSTER' up for inspection"
...
helm install kyma deploy/helm/kyma-engine -n "$NS" \
  ...
  --set-string env.KYMA_INGEST_MODE="staged" \
  --set-string env.KYMA_COMMIT_WINDOW_MS="1000" \
...
roll_ok=1
for d in kyma-edge kyma-committer kyma-worker; do
...
kubectl port-forward -n "$NS" svc/kyma "${LPORT}:8080" --address 127.0.0.1 >/tmp/kyma-roles-pf.log 2>&1 &
PF_PID=$!; disown "$PF_PID" 2>/dev/null || true
healthy=0
for i in $(seq 1 40); do curl -fsS "$BASE/health" >/dev/null 2>&1 && { healthy=1; break; }; sleep 2; done
[[ "$healthy" == "1" ]] && ok "Service serves /health" || { bad "/health unreachable"; cat /tmp/kyma-roles-pf.log; exit 1; }
...
kubectl delete pod -n "$NS" -l kyma.io/role=committer --grace-period=0 --force >/dev/null 2>&1 || true

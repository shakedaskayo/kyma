...
variable "compute_backend" {
  ...
  validation {
    ...
    error_message = "compute_backend must be \"fargate\" or \"eks\" (helm/local targets do not use this stack)."
  }
}
...
# Bring-your-own catalog (database_backend = "external").
...
# Pulumi terraform-module bridge can infer this module's schema — terraform
# >=1.15 rejects no-args module calls at init for truly required variables.
...
# Bring-your-own S3-compatible store (storage_backend = "external").

...
//! `pensieve-embed`; the backend (an ONNX model for fastembed) is built once and
//! shared process-wide via a `OnceCell` so the model loads at most once.

use std::sync::Arc;

use pensieve_embed::{EmbeddingBackend, EmbeddingConfig, EmbeddingProvider, RerankBackend};
...
/// `PENSIEVE_RERANK_MODEL` (e.g. `"bge-reranker-base"`); `PENSIEVE_RERANK_MODEL_PATH`
...
pub async fn shared_reranker() -> Option<Arc<dyn RerankBackend>> {
    RERANKER
        .get_or_init(|| async {
            let model = std::env::var("PENSIEVE_RERANK_MODEL").ok()?;
            if model.trim().is_empty() {
                return None;
            }
            let path = std::env::var("PENSIEVE_RERANK_MODEL_PATH").ok();
            match pensieve_embed::FastembedReranker::new(&model, path.as_deref()).await {
...
/// `pensieve-embed` feature is enabled are available; others return an error.
pub async fn build_embedding_backend(cfg: &EmbeddingConfig) -> Result<Arc<dyn EmbeddingBackend>> {
    match &cfg.provider {
        EmbeddingProvider::Fastembed { id, model_path } => {
            let b = pensieve_embed::FastembedBackend::new(id, model_path.as_deref())
                .await
        ...
        other => Err(MemoryError::Embed(format!(
            "embedding provider not enabled in this build: {other:?}; \
             rebuild pensieve-memory with the matching pensieve-embed feature"
        ))),
    }
}

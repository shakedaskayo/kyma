...
//! server mode (Postgres catalog) and `kyma local` execute, so this covers the
...
async fn source_summary_groups_by_provenance_source_and_realm() {
    ...
    // Two claude-code memories in realm "kyma" (distinct provenance blobs —
    ...
    assert_eq!(
        body["items"],
        json!([
            {"source": "claude-code", "realm": "kyma", "count": 2},
            {"source": "manual", "realm": "default", "count": 1},
        ]),
        "expected claude-code/kyma=2 + manual/default=1, archived dreaming excluded; got {body}",
    );
}

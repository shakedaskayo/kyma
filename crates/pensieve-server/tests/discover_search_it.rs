...
use pensieve_server::auth::{AuthLayerState, EnvAuthBackend, Role};
use std::sync::Arc;
use tower::ServiceExt;

/// Serializes the tests that depend on `PENSIEVE_DISCOVER_MAX_SOURCES`. cargo runs
...
fn build_app(state: pensieve_server::QueryState) -> axum::Router {
    let backend: Arc<dyn pensieve_server::auth::AuthBackend> = Arc::new(EnvAuthBackend::from_str(""));

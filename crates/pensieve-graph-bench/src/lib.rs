...
//! `pensieve-graph-testkit` (k-hop, shortest-path, synth-generation throughput)
//! on power-law graphs at 10k / 100k edges — and 1M edges when
//! `PENSIEVE_BENCH_LARGE=1` (kept off by default so `cargo bench` stays fast).
//!
...
//!
//! Run: `cargo bench -p pensieve-graph-bench` (smoke: `cargo bench -p
//! pensieve-graph-bench -- --test`).

...
//!
//! The harness is deliberately decoupled from kyma: anything that can answer
//! a [`ChainPattern`] with node-id tuples (the [`GraphEngineUnderTest`]
//! trait) can be differentially validated. [`HttpKymaEngine`] is the live
...
// =====================================================================
// Live-kyma engine over HTTP
// =====================================================================

/// Submits patterns as Cypher to a running kyma.
///
/// Wire contract (matches `crates/kyma-server/tests/cypher_query_it.rs` and
...
impl HttpKymaEngine {
    ...
    /// Engine location from env: `KYMA_BASE_URL` (default
    /// `http://127.0.0.1:8080`), `KYMA_DATABASE` (default `default`),
    /// `KYMA_TOKEN` (optional bearer token).
    pub fn from_env(graph: &str) -> Self {
        Self::new(
            std::env::var("KYMA_BASE_URL").unwrap_or_else(|_| "http://127.0.0.1:8080".into()),
            std::env::var("KYMA_DATABASE").unwrap_or_else(|_| "default".into()),
            graph,
            std::env::var("KYMA_TOKEN").ok(),
        )
    }
}

#[async_trait::async_trait]
impl GraphEngineUnderTest for HttpKymaEngine {
    async fn match_chain(&self, pattern: &ChainPattern) -> anyhow::Result<Vec<Vec<String>>> {
        let cypher = pattern.to_cypher();
        let mut req = self
            .client

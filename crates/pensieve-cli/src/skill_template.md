---
name: pensieve
description: The user's Pensieve context engine — durable memory + live data + a knowledge graph — via the `pensieve` CLI. RECALL prior decisions/preferences/conventions before answering, REMEMBER durable facts the user states, QUERY their live data (logs, traces, tables, code/graph) in plain English, and ENRICH the graph with entities. Use whenever a request may depend on past context or on the user's actual systems/data.
---

# Pensieve — the user's context engine

This machine is wired to **Pensieve**: the user's durable memory **+** live data **+** the
knowledge graph that links them, reachable through the `pensieve` CLI. Treat it as your
long-term memory of this user and project, *and* your window into their real systems.

**Setup check (once per machine):** run `pensieve status`. If it reports "no server
configured", ask the user to run `pensieve connect <their-server-url>`, then continue.
...
```bash
pensieve recall "how do we handle database migrations?"
```
...
```bash
pensieve remember "We deploy by tagging a release, then running make ship" --type procedure
...
Ask their Pensieve deployment anything about their data, in plain English; it streams the answer:

```bash
pensieve query "How many 500s on payments-svc in the last hour?"
pensieve query "What columns does the orders table have?"
pensieve query "Which services call auth-service?"
```

This runs live queries over the user's *actual* data — far better than guessing from
context. `pensieve query` is **read-only**. Add `--json` for the raw event stream.
...
- Anything requiring a write to the user's systems — `pensieve query` only reads.

## Troubleshooting

- `no server configured` → the user hasn't run `pensieve connect <url>` yet.
- `401` / `403` → the saved token expired: `pensieve connect <url> --token <new-token>`.
- Probe timeouts → the server is down or a model is loading; retry once `pensieve status` is OK.

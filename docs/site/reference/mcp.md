---
title: MCP
description: Pensieve's Model Context Protocol server — transports, tool catalog (23 tools), auth, and quick-start wiring for coding agents.
---

# MCP

Pensieve ships a built-in MCP server that exposes its full query, memory, and
graph surface as JSON-RPC 2.0 tools. Any MCP-capable client — Claude Code,
Cursor, Windsurf, or any custom agent — can drive pensieve through this interface
...
```bash
pensieve mcp   # run directly to test; agents invoke this as the MCP server process
```

`pensieve setup <agent>` writes (or merges into) the agent's MCP config file so
the agent spawns `pensieve mcp` automatically. See [`pensieve setup`](/reference/cli#pensieve-setup-agent-list-print).

Example config entry written by `pensieve setup claude-code` (`.mcp.json`):
...
When agents talk to a shared pensieve server instead of a local binary, they can
...
shares a single pensieve deployment, or when an agent runs in a remote environment
...
the credential store. They are not available over stdio / `pensieve mcp`.
:::
...
The stdio transport (`pensieve mcp`) runs under the local user's identity and
requires no token.
...
| `graph_traverse` | Traverse a graph stored as edges in a pensieve table using the KQL `graph-traverse` operator. Returns reachable nodes as (node, depth) pairs. |
...
These two tools are added when pensieve runs as a server and has access to the
credential store. They are **not available** over stdio (`pensieve mcp`).
...
- [CLI reference — `pensieve mcp` and `pensieve setup`](/reference/cli)

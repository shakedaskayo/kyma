---
title: Server compatibility — @kyma-ai/react
...
The SDK fetches `GET /v1/capabilities` once per `KymaProvider` mount and
caches the result for 5 minutes (`useKymaCapabilities`). The server returns a
...
specifically controls whether `KymaAgentChat` shows the chat UI or a "not
...
For servers that actively return `{ "agent": false }`, `KymaAgentChat` renders
...
The 404 response causes `useKymaCapabilities` to error after one retry. The
...
|---|---|---|
| **Live tail (SSE)** | Not in embeds | `discover_live` / `LiveSession` is available in `@kyma-ai/client` but not exposed in any embedded component today. Planned for a future SDK version. |
| **KymaAgentChat with scoped tokens** | 403 (fail-closed) | The agent's tool loop addresses databases internally; fine-grained enforcement is roadmap. Use a full-access token. |
...
No formal minimum server version is declared for `@kyma-ai/react` 0.x. The SDK

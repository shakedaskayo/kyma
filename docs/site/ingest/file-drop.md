---
title: File-drop
description: Drop NDJSON files into an object-store prefix; pensieve polls, hashes, and ingests. SHA256-keyed idempotency makes re-scans free.
---

# File-drop

Drop NDJSON files into a configured object-store prefix and pensieve picks
...
The watcher is opt-in. With `PENSIEVE_FILEDROP_ENABLED=1`, a worker starts
that polls the configured prefixes on the pensieve instance's own object
store (the same bucket the engine writes extents to).

| Variable                          | Default     | Notes                                                                         |
| --------------------------------- | ----------- | ----------------------------------------------------------------------------- |
| `PENSIEVE_FILEDROP_ENABLED`           | `0`         | Set `1` to start the watcher.                                                 |
| `PENSIEVE_FILEDROP_PREFIXES`          | `ingest`    | Comma-separated list of prefixes to scan each tick.                           |
| `PENSIEVE_FILEDROP_PREFIX`            | _(legacy)_  | Single-prefix form. Used only if `PENSIEVE_FILEDROP_PREFIXES` is unset.           |
| `PENSIEVE_FILEDROP_POLL_SECS`         | `5`         | Scan interval, applied to every prefix.                                       |
| `PENSIEVE_FILEDROP_AUTO_CREATE`       | `1`         | Auto-create the database and table on first file. Set `0` to require pre-existing tables. |
| `PENSIEVE_FILEDROP_SCHEMA_EVOLVE`     | `1`         | `ALTER TABLE ADD COLUMN` for unknown top-level keys.                          |
| `PENSIEVE_FILEDROP_DELETE_AFTER_INGEST`| `0`        | Delete the source object after a successful, non-replayed ingest.             |

Multiple prefixes let one pensieve instance host watchers for many
...
With pensieve running and the watcher enabled (the dev `docker-compose.yml`
sets `PENSIEVE_FILEDROP_ENABLED=1` and uses MinIO):

```bash
# Drop a file under ingest/default/orders/.
mc cp orders.ndjson pensieve/ingest/default/orders/2026-05-02-10.ndjson
```

Within `PENSIEVE_FILEDROP_POLL_SECS`, a log line confirms the ingest:

```
INFO pensieve_ingest_filedrop: filedrop: ingest complete
...
```
INFO pensieve_ingest_filedrop: filedrop: ingest complete rows=0 replayed=true
```
...
- **Delete-after-ingest race.** If
  `PENSIEVE_FILEDROP_DELETE_AFTER_INGEST=1` and the delete fails after a

---
title: Prometheus
description: The shipped reference connector. Scrapes a /metrics endpoint on a schedule, parses OpenMetrics / Prometheus text exposition, and lands one row per sample in the configured table.
---

# Prometheus

Prometheus is the reference implementation of the
[Connector framework](/connectors/framework). It does the smallest
...
[secret references](/connectors/framework#secrets-by-reference) —
typically `"$env:NAME"`.
...
The connector sends an `Accept` header advertising both OpenMetrics 1.0
...
[runner](/connectors/framework#scheduler-and-runner) treats as a tick
...
- **Endpoint returns HTML.** Parser error → `Permanent`. Connector keeps
...
- **Token rotates and `$env:NAME` no longer resolves.** Returns `Config`,
  the connector is disabled with `disabled_reason="token resolve: ..."`.
...
- The trait, registry, and admin API: [Framework](/connectors/framework).

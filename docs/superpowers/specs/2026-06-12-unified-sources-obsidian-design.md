...
| Sync engine | New `vault_sync.rs` in `pensieve-local`, modeled on `cc_sync.rs` but vault-flavored (frontmatter optional, recursive walk, Obsidian link syntax) | Generalizing `cc_sync.rs` itself would tangle Claude-specific loop-guard/writeback semantics into the vault path |
...
### 2. `ObsidianDataSource` (`crates/pensieve-datasources/src/obsidian.rs`)
...
- `run_once`: returns `Err(Permanent)` — defensive; the scheduler never ticks watcher rows. Registered in pensieve-local (and server registry, where it appears in the catalog; execution requires a node-side watcher).

### 3. Vault sync engine (`crates/pensieve-local/src/vault_sync.rs`)
...
### 4. Watcher manager (`crates/pensieve-local/src/source_watchers.rs`, spawned from `lib.rs` next to the cc-sync loop)
...
- New dep: `notify = "6"` in `pensieve-local`.
...
- **pensieve-ccmem**: wikilink normalization table tests (alias, heading, block, embed, dedupe, unterminated).
- **pensieve-local `vault_sync`** (pattern: `cc_sync_unit_tests.rs`, stub embedder + tempdir engine): ingest with/without frontmatter; idempotent rescan (skip on hash); edit → same node new version; delete → archived + un-archive on reappear; wikilink edges incl. alias + case-insensitive resolution; `.obsidian/` exclusion; realm naming.
- **pensieve-local `source_watchers`**: `reconcile_plan` unit tests (new row → start; disabled → stop; config change → restart; unchanged → no-op).
- **pensieve-datasources**: obsidian `validate_config`; catalog includes drive_model; create honors registry drive_model (admin_it).
...
- **Validation**: `cargo build && cargo clippy && cargo test` (workspace), `pnpm typecheck && pnpm test && pnpm build` (web), then a live `pensieve serve` pass against a real temp vault — add a note, edit it, link it, delete it — verifying memory nodes/edges/archive and the UI page end-to-end.

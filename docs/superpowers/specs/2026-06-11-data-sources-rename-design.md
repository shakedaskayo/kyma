...
1. **Full rename, clean break** — no `/v1/connectors` aliases, no deprecated CLI subcommand,
   no client-lib re-exports. Pensieve is v0.0.1; back-compat shims are dead weight.
...
| Today | Becomes |
|---|---|
| crate `pensieve-connectors` (`crates/pensieve-connectors/`) | crate `pensieve-datasources` (`crates/pensieve-datasources/`) |
| crate `pensieve-connector-core` | crate `pensieve-datasource-core` |
...
| CLI `pensieve connector <list\|add\|show\|pause\|resume\|remove\|trigger>` | `pensieve datasource <…>` |
| CLI `pensieve ingest status --connector` | `--datasource` |
...
| MCP tools `list_connectors`, `connector_read` (`crates/pensieve-server/src/agent/connector_tools.rs`) | `list_data_sources`, `data_source_read` (`datasource_tools.rs`) |
| capability field `Capabilities.connectors` (`pensieve-server/src/capabilities.rs:17`, serialized key `connectors`) + web `useCapability("connectors")` | `Capabilities.data_sources` / `useCapability("data_sources")` |
...
**crates.io:** the old `pensieve-connectors` / `pensieve-connector-core` names stay dormant at their
published 0.0.1; new names publish at the next version tag. Publishing is out of scope for this
change.

## 2. Database migration

New migration `027_data_sources_rename.sql` (next number in `crates/pensieve-catalog/migrations/`,
...
- `FiledropWatcher::run()` (pensieve-ingest-filedrop) and the cc-sync watcher (pensieve-local,
  `PENSIEVE_CC_WATCH`) register on startup (upsert by `(kind, node_id, config hash)`) and update
...
- CLI: `pensieve datasource list/add/show/pause/resume/remove/trigger` smoke tests.

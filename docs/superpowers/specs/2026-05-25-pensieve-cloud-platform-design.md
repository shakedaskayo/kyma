# Pensieve Cloud — Managed Multi-Tenant SaaS — Master Design
...
**Relationship to other specs:** This builds *on top of* the engine. It depends on `2026-05-19-pensieve-v1-production-readiness-design.md`, especially **A4 (multi-tenancy + security)** and **A2 (MCP surface)**. Where the engine's A4 isolation is not yet landed, the Cloud **gateway** compensates (defense in depth); see §6 dependency note.
...
Pensieve Cloud is a **managed, multi-tenant SaaS** where a developer can: sign up, create a project, point their stack's OTLP/REST telemetry at a Pensieve Cloud endpoint, and **wire their telemetry into Claude Code in under a minute** via a copy-paste `claude mcp add` command — then have their agent ask production questions in KQL/SQL over Arrow Flight. No infrastructure for the customer to run.
...
2. **Answers out:** dashboard hands the customer a one-line `claude mcp add pensieve --transport http https://<project>.mcp.getpensieve.cloud --header "Authorization: Bearer pensieve_…"`; their agent queries telemetry the same afternoon.

### Tenancy decision (the hinge)

**Launch pooled, keep a one-click silo path.** (AWS SaaS pool/bridge/silo model.)

- **Free / Pro tiers → Pooled.** One engine fleet; tenants separated by Pensieve-native `X-Database` + per-tenant S3 key prefix. Cross-tenant isolation enforced by **scoped STS session credentials** (a request scoped to tenant A *cannot* GET tenant B's prefix) + **hard query budgets / rate limits** for noisy-neighbor control. Best density and cost.
...
The migration pool→silo is a **deployment + data-repoint change, not a rewrite** — this is Pensieve's own architectural promise applied to tenancy. The design's job is to keep that promise cheap (§6, C6).
...
```
                         ┌─────────────────────────────────────────┐
   Customer browser ───► │  pensieve-cloud-dashboard (Next.js, Railway) │
                         │  Supabase Auth · usage · billing portal  │
                         └───────────────┬─────────────────────────┘
                                         │  control-plane API
                         ┌───────────────▼─────────────────────────┐
                         │  pensieve-cloud-api (control plane, Railway) │
   ...
   (OTLP, REST, Flight, MCP) ► ┌──────────────────────────────────┐
                               │  pensieve-gateway (auth proxy, Railway)│
                               ...
                               ┌──────────────▼─────────────────────┐
                               │  pensieve-bin engine fleet (Railway)    │   ← portable to AWS at scale (§6)
...
|---|---|---|---|
| `pensieve-cloud-dashboard` | Next.js + Supabase Auth | Railway | Signup, onboarding, API keys, MCP snippet, usage, billing portal, minimal query console |
| `pensieve-cloud-api` | TypeScript (Node) — see §7 decision | Railway | Control-plane CRUD, key issuance, Stripe webhooks, silo provisioning trigger |
| `pensieve-gateway` | Rust (reuse engine crates) — see §7 | Railway | Per-request authN, tenant resolution, scoped-credential minting, quotas, usage metering. Keeps the OSS engine clean. |
| `pensieve-bin` engine fleet | Existing Rust binary, containerized | Railway (→ AWS at scale) | Unmodified OSS engine; ingest + query + MCP |
| Control-plane DB | Supabase project **A** | Supabase (on AWS) | orgs, projects, api_keys, usage_events, subscriptions, audit_log |
| Engine catalog | Supabase project **B** | Supabase (on AWS) | Pensieve's Postgres catalog (manifests, stats, CAS) |
| Object storage | AWS S3 | AWS | Extents — the source of truth |
| Billing | Stripe | SaaS | Products, metered prices, invoices |
| DNS / edge | Cloudflare | SaaS | `getpensieve.cloud`, per-project subdomains, TLS |
...
**Why a gateway in front of the engine:** all multi-tenant *policy* (key auth, tenant→database mapping, scoped-credential minting, quota/rate enforcement, usage capture) lives in `pensieve-gateway`. The engine stays the stock OSS `pensieve-bin`. This means (a) the OSS engine never grows SaaS-specific code, (b) tenancy bugs are fixed in one place, (c) defense-in-depth: gateway enforces isolation *and* the engine enforces its own A4 isolation.
...
On project creation, `pensieve-cloud-api` performs an **idempotent, transactional** sequence:

1. Insert `org`/`project` rows (control-plane DB, project A).
2. Allocate a tenant identity: `tenant_id`, Pensieve `X-Database` name, S3 key prefix `s3://pensieve-prod-extents/<tenant_id>/`.
3. Register the database namespace in the engine catalog (project B).
4. Mint an **ingest token** and a **query/MCP token** (`pensieve_…`), store only hashes.
...
| **C1 Data plane** | 2–3 wk | Containerized `pensieve-bin`, engine + catalog (Supabase B) + S3 deployed on Railway; e2e ingest+query against managed infra; measured latency + cost envelope |
| **C2 Control plane** | 4–6 wk | Supabase Auth, control-plane schema (A), `pensieve-cloud-api`, `pensieve-gateway`, pooled tenancy, scoped STS, cross-tenant-denied tests |
...
| **C5 Hardening + scale** | 4–6 wk | Engine autoscaling, noisy-neighbor controls, catalog PITR + S3 versioning/replication DR, **dogfood Pensieve-on-Pensieve** observability + Grafana, secret rotation, pen-test, SOC-2-ready controls, SLOs + runbooks |
...
| Gateway language | **Rust (reuse engine crates)** | Hot path; reuse auth/Flight/token code from `pensieve-server`; keep OSS engine unmodified |
...
- **Selling observability means holding customers' sensitive telemetry.** Prompt bodies, tool args, and audit trails may contain secrets. TLS everywhere, scoped creds, encryption at rest (S3 SSE), audit logging (dogfooded into Pensieve), and a clear data-handling policy are table stakes, not C5 nice-to-haves.
...
- Containerize `pensieve-bin` as strict 12-factor (config/env only, no Railway lock-in).
...
- `pensieve-cloud-api`: project CRUD, transactional pooled-tenant provisioning, key issuance (hash-at-rest).
- `pensieve-gateway`: api-key authN, tenant resolution, scoped-STS minting, `X-Database` injection, quota/rate limit, usage capture.
...
- One-line `claude mcp add pensieve --transport http …` snippet generator (per project, copy button).
...
- Observability: **dogfood — Pensieve Cloud ingests its own telemetry into a Pensieve instance**; ship Grafana dashboards + alerts.

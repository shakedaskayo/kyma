# Kyma Agentic Memory — Master Design
...
Give **Ask Kyma** a real, configurable memory:
...
2. **Agentic Memory** — conversation turns (and other Kyma data) get consolidated into **memory nodes** that link to existing graph entities via `REFERENCES` edges, forming a "neural network" over the knowledge graph.
3. **Configurable memory pipelines** — defined abstractly against Kyma tables / graphs / namespaces / connectors (the Discover `Scope` model), with modes for session-turn ingestion, source ingestion, and autonomous "dreaming" (self-learning + consolidation). System-prompt-as-config; an LLM agent with memory tools does the work.
...
5. **Claude Code integration via hooks** — `kyma install-hooks` wires SessionStart/UserPromptSubmit → recall/inject and Stop/PostToolUse → cheap capture, so Claude Code automatically deposits conversational context into Kyma and pulls relevant context back out.
...
The deliverable is **end-to-end and runnable**: a user runs `kyma install-hooks`, works in Claude Code across sessions, and Kyma quietly captures turns; later, a `dream` pipeline consolidates them into memory nodes linked to repos/services/tables; the next session's SessionStart hook injects the relevant memories back into Claude Code's context — and the user can see and curate it all in the Memory page.
...
- **A bespoke vector store.** We reuse Kyma columnar storage + the existing `cosine_distance` UDF, not a new pgvector index or a memvid-style file format.
...
```
 Claude Code ── hooks ───────────────────────────────────────────────┐
   SessionStart / UserPromptSubmit → `kyma memory recall`  (READ)     │
   Stop / PostToolUse             → `kyma memory ingest-turn` (WRITE) │
                                                                      │ HTTP (Bearer)
 web/ (React)  Memory page · pipeline mgr · GraphView memory layer    │
                                                                      ▼
 kyma-server   /v1/agent/ask            (SSE, now session-aware)   ┌──────────────┐
        ...
        ┌──────────────┼───────────────────────────┬─────────────────────┤
        ▼              ▼                           ▼                     ▼
   kyma-memory     pipeline_runner            RecallService        memory agent tools
   (writer +       (reuses agent runner +     (cosine UDF +        save/recall/list/
    ...
    columnar)      prompt; dream scheduler)   rerank)              link/merge
        │              │                                                  │
        ▼              ▼                                                  ▼
   memory_nodes / memory_edges (Kyma columnar)  ──register──►  "memory" graph
...
- **Migration `015_memory_sessions.sql`:** `ALTER TABLE agent_sessions ADD COLUMN title TEXT, rolling_summary TEXT, summary_turn_index INT NOT NULL DEFAULT 0, source TEXT NOT NULL DEFAULT 'kyma'`; index on `last_active DESC`.
...
- **Rolling summary:** when `turn_count - summary_turn_index > N` (env `KYMA_SESSION_SUMMARY_EVERY`, default 12), a detached task summarizes the conversation and updates `agent_sessions`. Keeps the request path clean and bounds context growth.
- **New endpoints:** `GET /v1/agent/sessions`, `GET /v1/agent/sessions/:id`, `GET /v1/agent/sessions/:id/turns`, `DELETE /v1/agent/sessions/:id`.
- **CLI:** `kyma query --session <id>` / `--continue` (stores `last_session_id` in `~/.kyma/config.json`); `kyma sessions <list|show|turns|delete>`.
...
The `kyma-memory` crate + the columnar memory graph + memory agent tools.

- **Crate `kyma-memory`:** `types.rs` (`MemoryEntry`, `MemoryType{Fact,Decision,Preference,Learning,Summary}`, `MemoryStatus{Active,Background,Archived}`, `CreateMemory`, `MemoryListFilter`, `MemorySearchRequest`, `RecallHit`, `MemoryEdge`); `writer.rs` (`MemoryWriter`: build node/edge rows, embed content via `kyma-embed`, append via `kyma-ingest-core::WritePath` group-commit, register the `memory` graph on first write); `recall.rs` (`RecallService`: cosine-UDF query + filter + 0.7/0.3 re-rank + off-hot-path access event); `error.rs`.
...
- **Agent tools** (registered in `agent/tools.rs`, usable by both Ask Kyma and the ingestion agent): `save_memory`, `recall_memory`, `list_memories`, `update_memory_status`, `update_memory_importance`, `link_memory_to_entity`, `merge_memories`.
...
- **`POST /v1/memory/events`** — cheap synchronous insert, no LLM. Requires the `KYMA_TOKEN` bearer (min role Write) so `tenant_id` is populated. This is what Claude Code Stop/PostToolUse hooks write to via `kyma memory ingest-turn`.
- **`kyma memory ingest-turn`** reads the hook's stdin JSON (parsed defensively — whole payload stored, fields extracted best-effort) and POSTs it.
...
- **Endpoints:** `POST/GET /v1/memory/pipelines`, `GET/PATCH/DELETE /:id`, `POST /:id/run|pause|resume`, `GET /:id/status|runs`. CLI `kyma memory pipeline <create|list|run|status|…>`.
...
- **`POST /v1/memory/query`** with `mode: fast | agentic` and `filters` (namespace/realm, connector/source, memory_type, time range, importance_min, status).
  - **fast** — `kyma-memory::recall`: embed query → cosine-UDF ANN → enrich → filter → 0.7/0.3 re-rank → return memories + a compact `context` block. One embed + one query; suitable for hooks.
  - **agentic** — a short-budget agent (reuses runner) with recall + `graph_traverse` + discover tools that synthesizes/summarizes a `brief`.
- Default recall scope = current project realm ∪ `global`. CLI defaults to `fast`; `--agentic` opts into synthesis. `kyma memory recall|query`.

### M5 — Claude Code hooks integration

- **`kyma install-hooks`** merges (never clobbers) a `hooks` block into Claude Code `settings.json` and writes `HOOKS.md`. Map: `SessionStart` + `UserPromptSubmit` → `kyma memory recall … --fast --json` (emits `{"hookSpecificOutput":{"hookEventName":…,"additionalContext":…}}`, exit 0 → injected); `Stop` + `PostToolUse` → `kyma memory ingest-turn` (`async: true`, capture only). Optional `--transport http` form posts straight to the endpoints.
...
|---|---|---|---|
| Kyma columnar | `memory_nodes`, `memory_edges` | append + latest-wins dedup | first-class queryable, renders in GraphView |
...
- **Append-only write amplification** — keep access tracking off the hot path; rely on Kyma extent compaction for node-version churn.

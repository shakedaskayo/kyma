...
| MCP | Re-point `memory_search`/`recall_memory` to call `unified_search(memory)`; add a `search` tool (`data` mode) and a `graph_search` tool (`graph` mode) so agents get the full substrate. |

# Dreaming via the Kyma Skill — Design

**Date:** 2026-06-09
**Status:** Approved (autonomous — shared-substrate program, piece 2)
**Scope:** Deliver the dreaming procedure as a **kyma skill** to the spawned Claude CLI (filesystem skill-delivery), replacing the hardcoded `dreaming_prompt()` playbook. The skill is the playbook; MCP tools stay the actuators. Agent-agnostic.

## Program context
Piece 2 of the shared-substrate program (4→1→3→2). Pieces 4, 1, 3 landed. The user's framing: "Dreaming via the kyma skill instead of MCP tool-calling (needs Claude-CLI skill-delivery design)."

## Problem
Dreaming spawns the Claude CLI with a long hardcoded `dreaming_prompt()` (`dreaming.rs:252`) as the question + the kyma MCP server, and **delivers zero skills** (`run_via_claude_cli`, `dreaming.rs:1022`). Only ADK agents get skills (system-prompt injection via `runner.rs::compose_system_prompt`); the Claude-CLI path has no skill delivery. The dreaming playbook is therefore frozen in Rust, can't be iterated like a skill, and doesn't compose with the user's other skills.
...
| Dreaming playbook | Moves from the Rust `dreaming_prompt()` string into a **`kyma-dreaming` SKILL.md** (the PHASE 1 review / PHASE 2 gap-fill / PHASE 3 graph-wiring procedure). The Rust prompt becomes a thin trigger: "Run a memory-consolidation (dreaming) pass; follow the `kyma-dreaming` skill." Budgets/mode/realm-scope still injected as short structured context. |
| Skills composed | The dreaming run delivers `kyma-dreaming` + the tenant's **enabled** kyma skills (so dreaming benefits from the same skills agents use). Reuse `skills::discover_all` + the enabled-set store. |
| MCP | Unchanged — the kyma MCP server is still wired (strict) as the actuator; the skill tells the agent which MCP tools to use. "Via the skill instead of MCP tool-calling" = the *playbook* comes from a skill, not a hardcoded prompt; tools remain. |
...
### 1. Skill-delivery module — `crates/kyma-server/src/agent/skill_delivery.rs` (new)
```rust
...
```
- `SkillDoc { name, body }` — from `skills::discover_all()` filtered to the enabled set, plus the built-in `kyma-dreaming`.
...
- Build a **thin** trigger prompt (mode + budgets + realm-scope as short structured context) that says to follow the `kyma-dreaming` skill.
...
- A malformed/oversized enabled skill → skip that skill (warn), still deliver `kyma-dreaming`.
- `claude` binary not found → unchanged (the user's `locate_binary` WIP improves this independently).

## Testing
- **skill_delivery unit:** `deliver_to_workdir([{name:"kyma-dreaming",body}])` writes `<workdir>/.claude/skills/kyma-dreaming/SKILL.md` with the body; multiple skills each land in their own dir; the `TempDir` exists until dropped.
- **dreaming wiring unit:** the skill set passed to delivery includes `kyma-dreaming` + the enabled skills (mock the enabled-set store); the thin prompt references the skill and carries the budgets/mode; on a forced delivery error, the code path selects the hardcoded `dreaming_prompt()` fallback. (Don't spawn a real `claude` — assert the assembled inputs, mirroring how existing dreaming tests assert the prompt.)
- **skill content:** `kyma-dreaming` SKILL.md parses (valid frontmatter `name`/`description`) and contains the three phases + the connector-read-budget rule.
- Existing dreaming tests still pass.

## Out of scope / deferred
- A bundled (on-disk) copy of `kyma-dreaming` in the plugin (v1 uses the embedded const).
...
## File touch list
- `crates/kyma-server/src/agent/skill_delivery.rs` (new) + `agent/mod.rs` (`pub mod`).
- `crates/kyma-server/src/agent/dreaming.rs` (gather + deliver + thin prompt + `cwd`; keep `dreaming_prompt` as fallback). Possibly a new `dreaming_skill.rs` for the embedded `kyma-dreaming` const.
- (Dep) `tempfile` in `kyma-server/Cargo.toml` if not already present.

...
**Co-working note:** the user is actively editing `KymaExplore.tsx` — Task 4 touches it minimally + carefully; Tasks 1–3 avoid it. Never stage other uncommitted files.

---

### Task 1: `kyma-kql` Cypher → KQL translator (the core; pure + fully unit-testable)

**Files:** `crates/kyma-kql/src/cypher.rs` (new), `crates/kyma-kql/src/lib.rs` (re-export).

- [ ] **Step 1** — Read `crates/kyma-kql/src/parser.rs` `op_make_graph` (~777) and `op_graph_match` (~804) to learn the EXACT `make-graph`/`graph-match`/`project` syntax to emit, and `crates/kyma-kql/src/lib.rs` for `ParseError`, `kql_to_sql_with_schemas`, `SchemaMap`.
- [ ] **Step 2** — Failing tests in `cypher.rs` `#[cfg(test)]`. Define a fixture `GraphBinding { edge_table:"github_edges", node_table:"github_nodes", id_col:"id", src_col:"src", dst_col:"dst", type_col:"type", label_col:"labels" }`. Assert `cypher_to_kql(...)` emits the expected KQL for:
  ...
  - For ≥2 representative cases, also assert the emitted KQL compiles: `kql_to_sql_with_schemas(&kql, &SchemaMap::default()).is_ok()` (or with a minimal schema map). Run `cargo test -p kyma-kql cypher` → FAIL.
...
- [ ] **Step 4** — `cargo test -p kyma-kql cypher` (pass) + `cargo build -p kyma-kql`. Commit: `feat(kql): Cypher → KQL translator (single-hop MATCH subset)`.

---

### Task 2: Server `application/x-cypher` routing + graph resolution

**Files:** `crates/kyma-server/src/lib.rs` (the `query_handler` content-type branch ~810).

- [ ] **Step 1** — Read the content-type branch + how `x-database` is read (~684) + how graphs are resolved (`catalog.get_graph` / `graph_handler::resolve_with`). Read `kyma_core::catalog::GraphSpec` for the column-role field names.
- [ ] **Step 2** — Failing integration test (`crates/kyma-server/tests/`, mirroring an existing `/v1/query` test if present; else a focused handler test): seed a stored graph; POST `/v1/query` with `content-type: application/x-cypher`, `x-graph: <db>/<graph>`, body `MATCH (a)-[r]->(b) RETURN a.name LIMIT 5` → 200 with rows; missing `x-graph` + ambiguous → 400; bad Cypher → 400 with the ParseError message. (If full HTTP harness is heavy, test the extracted resolution+translate fn directly.)
- [ ] **Step 3** — Implement: extract a helper `fn resolve_graph_binding(catalog, tenant, x_graph, in_scope_graphs) -> Result<GraphBinding, Resp>`; in `query_handler` add `else if content_type.starts_with("application/x-cypher") { let binding = resolve_graph_binding(...)?; let kql = kyma_kql::cypher_to_kql(&raw, &binding).map_err(400)?; let sql = kyma_kql::kql_to_sql_with_schemas(&kql, &schemas).map_err(400)?; ("cypher", sql) }`. Map `GraphSpec` → `GraphBinding` (id_col, label_col, src_col, dst_col, type_col, node_table, edge_table).
- [ ] **Step 4** — `cargo test -p kyma-server` (new test passes; existing query tests pass) + build. Commit: `feat(server): route application/x-cypher through cypher_to_kql + graph resolution`.

---

### Task 3: Smart-input detection + client wiring (no KymaExplore yet)
...
- [ ] **Step 3** — `pnpm --filter @kyma-ai/react test` + `pnpm --filter @kyma-ai/client typecheck`. Commit: `feat(explore+client): detect Cypher in the smart input + x-cypher wiring`.

---

### Task 4: `KymaExplore.tsx` dispatch + badge (minimal, careful, on co-working WIP)

**Files:** `packages/react/src/explore/KymaExplore.tsx`.

- [ ] **Step 1** — `git stash list` / `git diff --stat KymaExplore.tsx` first to SEE the user's current WIP; work on top of it, do not revert it. Read the current `run()` dispatch + mode badge.
...
- [ ] **Step 3** — `pnpm --filter @kyma-ai/react test` + typecheck. Manual: typing `MATCH (a)-[r]->(b) RETURN a` shows the `Cypher` badge and runs. Commit (ONLY KymaExplore.tsx, and ONLY the cypher additions — do not sweep the user's unrelated WIP into the commit; `git add -p` if needed): `feat(explore): Cypher mode in the smart input`.
...
- Co-working safety (KymaExplore last, `git add -p`) → Task 4. ✓

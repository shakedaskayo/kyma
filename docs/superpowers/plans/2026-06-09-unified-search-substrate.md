...
**Tech Stack:** Rust (pensieve-server, pensieve-mcp, DataFusion), TS (@pensieve-ai/client, @pensieve-ai/react).

**Preserve:** the uncommitted WIP in `search.rs` (concurrency limiter + per-source mem budget) and `PensieveExplore.tsx` (`autoRun`) — do NOT revert; build on top.

---

### Task 1: Split `search.rs` into a `search/` module + envelope types (refactor, no behavior change)

**Files:** Move `crates/pensieve-server/src/search.rs` → `crates/pensieve-server/src/search/mod.rs`; add `crates/pensieve-server/src/search/types.rs`.

- [ ] **Step 1** — `git mv crates/pensieve-server/src/search.rs crates/pensieve-server/src/search/mod.rs`. Confirm `mod search;` in `lib.rs` still resolves (Rust treats `search/mod.rs` as the module). Build: `cargo build -p pensieve-server` → clean. This preserves the WIP diff (git mv keeps it).
- [ ] **Step 2** — Add `crates/pensieve-server/src/search/types.rs` with the envelope types from the spec (`SearchMode`, `UnifiedHit`, `UnifiedSearchResponse`, and a `UnifiedSearchRequest`). Derive `Serialize`/`Deserialize`. `SearchMode` defaults to `Data` and deserializes from `"data"|"memory"|"graph"` (serde rename_all lowercase; `#[serde(default)]`). Add `pub mod types;` to `search/mod.rs`. Build clean.
- [ ] **Step 3** — Unit test in `types.rs`: a `SearchBody`-style JSON with no `mode` deserializes to `SearchMode::Data`; `"memory"`/`"graph"` parse; unknown → error. Run `cargo test -p pensieve-server search::types`. Commit: `refactor(search): split into search/ module + unified envelope types`.

---

### Task 2: `unified_search()` with Data mode + re-point the HTTP handler (backward-compatible)

**Files:** `crates/pensieve-server/src/search/unified.rs` (new), `crates/pensieve-server/src/search/mod.rs`.

- [ ] **Step 1** — Write a failing parity test (`search/unified.rs` `#[cfg(test)]` or an integration test): given a fixture DB+table, `unified_search(ctx, {mode:Data, query, scope})` returns hits whose `(source,row)` match the current `search_handler` data path. (Build the fixture the way `search/mod.rs` tests or `pensieve-catalog-sqlite` harness do; if no in-crate harness, assert the mapping from the existing per-source results to `UnifiedHit` instead.)
...
- [ ] **Step 4** — Run the existing search tests + the new parity test: `cargo test -p pensieve-server search`. Confirm the legacy `/v1/search` response is unchanged (add a serialization snapshot test asserting no `mode`/`context` keys appear for a data response). Commit: `feat(search): unified_search() dispatcher with data mode (backward-compatible)`.

---

### Task 3: Memory mode — delegate to `retrieve()` (parity, no regression)

**Files:** `crates/pensieve-server/src/search/unified.rs`.

- [ ] **Step 1** — Failing test: `unified_search(ctx,{mode:Memory,query,realms,...})` returns hits that match a direct `retrieve(&shared, &RetrieveRequest{...})` call (same ids, same order) for a seeded memory store, and propagates `context` + `linked`. (Seed via the `MemoryWriter` test harness from `pensieve-memory/tests/file_candidates_it.rs`.)
...
- [ ] **Step 3** — Run `cargo test -p pensieve-server search`. Parity test green. Commit: `feat(search): memory mode delegates to retrieve() (rich recall preserved)`.

---

### Task 4: Graph mode — delegate to graph provider search

**Files:** `crates/pensieve-server/src/search/unified.rs`; reuse `graph_handler.rs` resolution.
...
- [ ] **Step 3** — `cargo test -p pensieve-server search`. Commit: `feat(search): graph mode delegates to graph provider search`.

---

### Task 5: MCP — route memory tools through the substrate + add `search`/`graph_search` tools

**Files:** `crates/pensieve-server/src/agent/memory_tools.rs`, `crates/pensieve-mcp/src/tools.rs`, bundled skill docs.
...
- [ ] **Step 3** — `PensieveExplore.tsx`: keep `data` the default for the Search mode (current UX). Ensure the result renderer tolerates the new optional fields (no crash if `row` is absent for memory/node hits — guard the table render). Preserve the `autoRun` WIP. (A memory/graph mode chip is OPTIONAL — add only if < ~30 lines; else leave a `// TODO(piece-1 follow-up): memory/graph search chip` and ship data-compatible.)
- [ ] **Step 4** — `pnpm --filter @pensieve-ai/client test` (if present) + `pnpm --filter @pensieve-ai/react test`. Commit: `feat(client+react): search mode + unified envelope (data default preserved)`.

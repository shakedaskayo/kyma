...
This project is wired to **Pensieve**: the user's durable memory **+** live data **+** the
knowledge graph that links them. The `pensieve-memory` plugin bundles an MCP server named
`pensieve` (memory + data + graph tools) and **hooks** that automatically capture each session
...
Call `recall_memory` (MCP server `pensieve`) before answering anything that could depend
...
The same `pensieve` MCP server exposes `explore_schema`, `describe_table`, `run_kql`,
...
Call `ingest_entity` (MCP server `pensieve`) to mint a **virtual resource/entity** — a service,
...
`recall_memory`; it's idempotent per `(realm, kind, name)`. The `/pensieve-ingest` command guides
...
- Memory writes go to the user's own Pensieve server; treat them as durable and shared.

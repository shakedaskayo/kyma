...
# it never delays shutdown; the pensieve agent (which owns save_memory) does the work.
set -u
...
have jq || exit 0
[ "$PENSIEVE_CC_DISTILL" = "1" ] || exit 0
[ "$PENSIEVE_CC_CAPTURE" = "off" ] && exit 0
...
if [ "$PENSIEVE_CC_FILE_SYNC" = "1" ]; then
  ( pensieve sync --cc-only >/dev/null 2>&1 & ) >/dev/null 2>&1
fi

[ -n "$tp" ] && [ -f "$tp" ] || exit 0

# Feed a bounded transcript tail to `pensieve distill`, fully detached.
( tail -n 600 "$tp" 2>/dev/null \
    | pensieve distill --session "$sid" --realm "$realm" >/dev/null 2>&1 & ) >/dev/null 2>&1
exit 0

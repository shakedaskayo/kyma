...
text=""
[ "$PENSIEVE_CC_CAPTURE" = "full" ] && text="$(pensieve_clean "$prompt")"
...
[ -z "$prompt" ] && exit 0
mem="$(pensieve recall "$prompt" --realm "$realm" --limit "$PENSIEVE_CC_RECALL_LIMIT" 2>/dev/null)"
if [ -n "$mem" ]; then
  ctx="$(printf 'Relevant Pensieve memories (realm: %s) — consider before answering:\n%s' "$realm" "$mem")"
  jq -nc --arg c "$ctx" \
